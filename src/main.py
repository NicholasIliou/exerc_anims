...
def main(args: List[str] = None) -> int:
    ...
    # Warn if using multiple workers with GPU - not recommended for single GPU
    if parsed_args.workers > 1 and device == "cuda":
        if verbose:
            print(f"\nNote: Using --workers {parsed_args.workers} with GPU.")
            print("  For single GPU, sequential processing is usually faster.")
            print("  Using sequential GPU processing (each video processed one at a time).")
        # Force sequential for GPU to avoid memory conflicts
        effective_workers = 1
    else:
        effective_workers = parsed_args.workers
    
    if effective_workers > 1 and len(video_loader) > 1:
